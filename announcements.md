---
description: Here you can find all announcements for Project Confluence
icon: megaphone
layout:
  width: default
  title:
    visible: true
  description:
    visible: true
  tableOfContents:
    visible: true
  outline:
    visible: true
  pagination:
    visible: true
  metadata:
    visible: true
  tags:
    visible: true
---

# Announcements

{% updates format="full" %}
{% update date="2026-04-29" %}
## Microsoft Licence Migration Project - Important Update

Dear Colleagues,

The licence migration project team is delighted to announce that we are approx. 70% complete with our Email and OneDrive migrations from WIT to SETU. This level progress to down to the efforts of the project team and also the positive engagement from those successfully migrated.



**Important announcements to note:**

* If you have not been migrated to date, please continue to use your WIT Email and OneDrive accounts until notified by the Project Confluence team. This communication will only come from the managed mailbox of [ProjectConfluence@setu.ie](mailto:ProjectConfluence@setu.ie) so please monitor your WIT mailbox regularly for your migration date.
* MS Teams and SharePoint have NOT been migrated yet. The project will communicate go live dates for these migrations by the end of this week.



**Support**

If you experience any issues after the migration, please contact [ProjectConfluence@setu.ie](mailto:ProjectConfluence@setu.ie)

Thank you for your continued support.

<figure><img src=".gitbook/assets/SETU + Confluence (1).jpg" alt=""><figcaption></figcaption></figure>
{% endupdate %}

{% update date="2026-03-27" %}
## Email & OneDrive Migrations Update & Recognition

Dear Colleagues,

The Project Confluence team recently took some time out of the email and OneDrive migrations work to reflect on the significant progress made to date with some sweet treats at Stanka’s. With the project moving closer to the halfway stage, the project team would like to take the opportunity to thank everyone for their engagement and support to date.

As we progress with the email and OneDrive migrations, can people who are yet to do migrated please delete their deleted items as this does elongate the process if not complete.

Finally, a delivery date for the MS Teams and SharePoint migrations from WIT to SETU will be communicated in the coming weeks.

<figure><img src=".gitbook/assets/test.png" alt=""><figcaption></figcaption></figure>

Missing from photo are Dave Carthy and Eoghan Kinane.

If you have any questions in relation to this project, please contact [ProjectConfluence@setu.ie](mailto:ProjectConfluence@setu.ie)

Thank you again for your continued support.
{% endupdate %}

{% update date="2026-03-16" %}
## CRM Dynamics

Dear Colleagues,

The Project Confluence team is delighted to announce that our CRM Dynamics tenant migration from WIT to SETU is successfully complete as of last Monday 9<sup>th</sup> March.

The project team worked closely with our migration partners, Storm Technology, to define scope, assess end user impact, design the implementation plan and execute the migration into the SETU production environment.

This is a major milestone in the overall system integration roadmap for the University and would like to take the opportunity to thank all shareholders involved in making this key ICT project happen over the past few months.

Finally, a special mention to Dave Carthy for leading this initiative out and ensuring a smooth path to live throughout the lifecycle of the project.

If you have any questions in relation to the above message, please contact [ProjectConfluence@setu.ie](mailto:ProjectConfluence@setu.ie)

Thank you again for your continued support.

<figure><img src=".gitbook/assets/SETU + Confluence (2).png" alt=""><figcaption></figcaption></figure>
{% endupdate %}
{% endupdates %}
